...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	  ...
	  if(htim->Instance==TIM3) sensor_flag=1;  //100ms
}
...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start_IT(&htim3);// tim 3 for every 100ms
	  HAL_TIM_Base_Start_IT(&htim2);   // counter every 1 us
      HAL_UART_Receive_IT(&huart1,&rx_byte,1);
	  EV_Init(&ev);// ev intialization
	  HCSR04_Init();  // ultrasonic
      ADAS_Init(&adas);
      Fault_Init(&flt);// flag and active = 0
	  ...
	  while (1)
	  {
	      ...
	      if (flag_ev) {
            ...
            if (sensor_flag==1) // 100ms tick{
            {
            	...
            	Shell_Process();// wait for data and process the cmnd
