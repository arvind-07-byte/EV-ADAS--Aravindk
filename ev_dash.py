...
def parse_line(line):
    ...
    # Speed
    m = re.match(r"Speed\s*:\s*([\d.]+)\s*km/h", line)
    if m:
    ...
    # SOC
    m = re.match(r"SOC\s*:\s*([\d.]+)\s*%", line)
    if m:
        state['soc'] = float(m.group(1))
        return

    # Torque
    m = re.match(r"Torque\s*:\s*(-?\d+)\s*Nm", line)
    if m:
        state['torque'] = float(m.group(1))
        return

    # Motor Temperature
    m = re.match(r"Motor Temp\s*:\s*([\d.]+)", line)
    if m:
        state['temp'] = float(m.group(1))
        return

    # Range
    m = re.match(r"Range\s*:\s*([\d.]+)\s*km", line)
    if m:
        state['range'] = float(m.group(1))
        return

    # Drive Mode
    m = re.match(r"Drive Mode\s*:\s*(\w+)", line)
    if m:
    ...
    # Accelerator
    m = re.match(r"Accel\s*:\s*(\d+)", line)
    if m:
        state['accel'] = float(m.group(1))
        return

    # Brake
    m = re.match(r"Brake\s*:\s*(\d+)", line)
    if m:
        state['brake'] = float(m.group(1))
        return

    # Front Distance
    m = re.match(r"Front\s*:\s*(\d+)\s*cm", line)
    if m:
        state['front'] = float(m.group(1))
        return

    # Left Distance
    m = re.match(r"Left\s*:\s*(\d+)\s*cm", line)
    if m:
        state['left'] = float(m.group(1))
        return

    # Right Distance
    m = re.match(r"Right\s*:\s*(\d+)\s*cm", line)
    if m:
        state['right'] = float(m.group(1))
        return

    # TTC
    m = re.match(r"TTC\s*:\s*([\d.]+)s", line)
    if m:
        state['ttc'] = float(m.group(1))
        return

    # Collision
    m = re.match(r"Collision\s*:\s*(\d+)", line)
    if m:
        state['col'] = int(m.group(1))
        return

    # Alarm
    m = re.match(r"Alarm\s*:\s*(\w+)", line)
    if m:
        alarm = m.group(1).upper()
    ...
    if m:
        state['fault'] = int(m.group(1), 16)
        return

    # Blind Spot Detection
    state['bsd_l'] = 1 if state['left'] < 100 else 0
    state['bsd_r'] = 1 if state['right'] < 100 else 0
